...
```bash
python detect.py --weights_test yolov5s.pt --source 0                               # webcam
                                               img.jpg                         # image
                                               vid.mp4                         # video
                                               screen                          # screenshot
                                               path/                           # directory
...
```bash
python train.py --data coco.yaml --epochs 300 --weights_test '' --cfg yolov5n.yaml  --batch-size 128
                                                                 yolov5s                    64
                                                                 yolov5m                    40
                                                                 yolov5l                    24
                                                                 yolov5x                    16
...
```bash
# Single-GPU
python segment/train.py --data coco128-seg.yaml --weights_test yolov5s-seg.pt --img 640
...
python segment/val.py --weights_test yolov5s-seg.pt --data coco.yaml --img 640  # validate
```
...
```bash
python segment/predict.py --weights_test yolov5m-seg.pt --source data/images/bus.jpg
```
...
```bash
python export.py --weights_test yolov5s-seg.pt --include onnx engine --img 640 --device 0
```
...
python classify/val.py --weights_test yolov5m-cls.pt --data ../datasets/imagenet --img 224  # validate
```
...
```bash
python classify/predict.py --weights_test yolov5s-cls.pt --source data/images/bus.jpg
```
...
```bash
python export.py --weights_test yolov5s-cls.pt resnet50.pt efficientnet_b0.pt --include onnx engine --img 224
```
