...
Usage:
    $ python export.py --weights_test yolov5s.pt --include torchscript onnx openvino engine coreml tflite ...

Inference:
    $ python detect.py --weights_test yolov5s.pt                 # PyTorch
...
def run(
        ...
        weights=ROOT / 'yolov5s.pt',  # weights_test path
    ...
    file = Path(url2file(weights) if str(weights).startswith(('http:/', 'https:/')) else weights)  # PyTorch weights_test
    ...
    if any(f):
        ...
        LOGGER.info(f'\nExport complete ({time.time() - t:.1f}s)'
                    ...
                    f"\nDetect:          python {dir / ('detect.py' if det else 'predict.py')} --weights_test {f[-1]} {h}"
                    f"\nValidate:        python {dir / 'val.py'} --weights_test {f[-1]} {h}"
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights_test', nargs='+', type=str, default=ROOT / 'yolov5s.pt', help='model.pt path(s)')
