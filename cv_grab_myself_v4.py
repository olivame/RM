...
class lidar_nudt:
    def __init__(self):
        ...
        if self.enemy_is_blue:
            self.pos_data = [1800, 100, 1500, 850, 1950, 1350, 1865, 700, 1650, 100, 2250, 750]
        else:
            # 敌方是红方
            self.pos_data = [1000, 1300, 800, 1050, 850, 150, 1125, 1400, 1150, 1400, 570, 750]
        # 定时器一直发送
    ...
    def timer_callback(self,event):
        if self.enemy_is_blue:
            # 我方为红方：英雄靠后（2300， 250）靠前（1800， 100）；步兵3号打前哨站（1650，100），四号增益点（1950，1350），五号飞坡点（1600， 1450）
            ...
            t2 = time.time()
            t_wasted = t2 - t1  # 浪费时间在10**（-5）量级
            time.sleep(self.delaytime - t_wasted)
        else:
            # 我方为蓝方：英雄靠后（500， 1250）靠前（1000， 1400）；步兵3号打前哨站（1150，1400），四号增益点（850，150），五号飞坡点（1200， 50）
            ...
            # 一号英雄
            if (t1 - t0) <= 180:
                self.pos_data[0] = 1000
                self.pos_data[1] = 1300
            # 二号工程
            if (t1 - t0) <= 80:
                self.pos_data[2] = 800
                self.pos_data[3] = 1050
            if (t1 - t0) >= 80:
                self.pos_data[2] = 1300
                self.pos_data[3] = 650
            # 三号步兵
            if (t1 - t0) <= 160:
                self.pos_data[4] = 850
            ...
            # 四号步兵
            if (t1 - t0) <= 160:
                self.pos_data[6] = 1125
                self.pos_data[7] = 1400
            # 五号步兵
            # if (t1 - t0) <= 120:
            #     self.pos_data[4] = 1150
            #     self.pos_data[5] = 1400
            # print(f"发送坐标为:{self.pos_data}")
