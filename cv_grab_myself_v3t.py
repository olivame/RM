...
class Communicator:
    def __init__(self):
        ...
        self.ROBOT_BLOODS = []
        # 触发双倍易伤
        self.double1_R_datapack = b'\xa5\x07\x00\x00\xb9\x01\x03!\x01\t\x00\x80\x80\x01f\x86'
        self.double2_R_datapack = b'\xa5\x07\x00\x00\xb9\x01\x03!\x01\t\x00\x80\x80\x02\xfd\xb4'
        self.double1_B_datapack = b'\xa5\x07\x00\x00\xb9\x01\x03!\x01m\x00\x80\x80\x01\xc5\n'
        self.double2_B_datapack = b'\xa5\x07\x00\x00\xb9\x01\x03!\x01m\x00\x80\x80\x02^8'

        self.cmd_id_blood = int(0x0003) #转化为整数
    ...
    def lmap_interaction_send(self, pos_data, seq):
        ...
        for i in range(0, len(pos_data), 2):
            x = int(pos_data[i]).to_bytes(2, self.storage_mode, signed=True)
            y = int(pos_data[i+1]).to_bytes(2,self.storage_mode,signed=True)
            data  += x + y
        return self.package_data(cmd_id, seq, data)
...
class lidar_nudt:
    def __init__(self):
        ...
        if self.enemy_is_blue:
            self.pos_data = [1600, 100, 2000, 450, 1500, 100, 1950, 1350, 1950, 1350, 2250, 750]
        else:
            # 敌方是红方
            self.pos_data = [1200, 1400, 800, 1050, 1300, 1400, 850, 150, 850, 150, 570, 750]
        self.pos_data_cache = []  # 存储上一帧所有兵种位置
        # 临时ideas
        # 1号英雄
        self.conf_change_1 = 0
        self.conf_keep_1 = 0
        self.decision_change_1 = False
        # 2号工程
        self.conf_change_2 = 0
        self.conf_keep_2 = 0
        self.decision_change_2 = False
        # 3号平步
        self.conf_change_3 = 0
        self.conf_keep_3 = 0
        self.decision_change_3 = False
        # 四号步兵
        self.conf_change_4 = 0
        self.conf_keep_4 = 0
        self.decision_change_4 = False
        # 五号步兵
        self.conf_change_5 = 0
        self.conf_keep_5 = 0
        self.decision_change_5 = False
        # 定时器一直发送
                ...
                if len(results_second) > 0:
                    ...
                    if self.enemy_is_blue:
                        if label_second == "B1" and self.T1:
                            ...
                            self.pos_data[0] = (mean_cloud[0] - self.lidar_pos_x)*100
                            self.pos_data[1] = (mean_cloud[1] + self.lidar_pos_y)*100
                        elif label_second == "B2" and self.T1:
                            # mean_cloud[0] = (mean_cloud[0] / 0.34202 - mean_cloud[2] / 0.93969) / 0.36397
                            self.pos_data[2] = (mean_cloud[0] - self.lidar_pos_x)*100
                            self.pos_data[3] = (mean_cloud[1] + self.lidar_pos_y)*100
                        elif label_second == "B3" and self.T1:
                            # mean_cloud[0] = (mean_cloud[0] / 0.34202 - mean_cloud[2] / 0.93969) / 0.36397
                            self.pos_data[4] = (mean_cloud[0] - self.lidar_pos_x)*100
                            self.pos_data[5] = (mean_cloud[1] + self.lidar_pos_y)*100
                        elif label_second == "B4" and self.T1:
                            # mean_cloud[0] = (mean_cloud[0] / 0.34202 - mean_cloud[2] / 0.93969) / 0.36397
                            self.pos_data[6] = (mean_cloud[0] - self.lidar_pos_x)*100
                            self.pos_data[7] = (mean_cloud[1] + self.lidar_pos_y)*100
                        elif label_second == "B5" and self.T1:
                            # mean_cloud[0] = (mean_cloud[0] / 0.34202 - mean_cloud[2] / 0.93969) / 0.36397
                            self.pos_data[8] = (mean_cloud[0] - self.lidar_pos_x)*100
                            self.pos_data[9] = (mean_cloud[1] + self.lidar_pos_y)*100
                        elif label_second == "B7" and self.T1:
                            # mean_cloud[0] = (mean_cloud[0] / 0.34202 - mean_cloud[2] / 0.93969) / 0.36397
                            self.pos_data[10] = (mean_cloud[0] - self.lidar_pos_x)*100
                            self.pos_data[11] = (mean_cloud[1] + self.lidar_pos_y)*100
                            self.guess_7_state = 1
                    else:
                        if label_second == "R1" and self.T1:
                            self.pos_data[0] = (self.x_length - (mean_cloud[0] - self.lidar_pos_x))*100
                            self.pos_data[1] = (self.y_length - mean_cloud[1] - self.lidar_pos_y)*100
                        elif label_second == "R2" and self.T1:
                            self.pos_data[2] = (self.x_length - (mean_cloud[0] - self.lidar_pos_x))*100
                            self.pos_data[3] = (self.y_length - mean_cloud[1] - self.lidar_pos_y)*100
                        elif label_second == "R3" and self.T1:
                            self.pos_data[4] = (self.x_length - (mean_cloud[0] - self.lidar_pos_x))*100
                            self.pos_data[5] = (self.y_length - mean_cloud[1] - self.lidar_pos_y)*100
                        elif label_second == "R4" and self.T1:
                            self.pos_data[6] = (self.x_length - (mean_cloud[0] - self.lidar_pos_x))*100
                            self.pos_data[7] = (self.y_length - mean_cloud[1] - self.lidar_pos_y)*100
                        elif label_second == "R5" and self.T1:
                            self.pos_data[8] = (self.x_length - (mean_cloud[0] - self.lidar_pos_x))*100
                            self.pos_data[9] = (self.y_length - mean_cloud[1] - self.lidar_pos_y)*100
                        elif label_second == "R7" and self.T1:
                            self.pos_data[10] = (self.x_length - (mean_cloud[0] - self.lidar_pos_x))*100
                            self.pos_data[11] = (self.y_length - mean_cloud[1] - self.lidar_pos_y)*100
                            self.guess_7_state = 1
                self.T1 = True
                self.guess_7_decision = self.guess_7_state
                self.guess_7_state = 0
        t_test2 = time.time()
        print(f"处理一帧耗时为：{t_test2 - t_test1}")
        img = annotator.result()
        cv2.namedWindow("Detect", 0)
        cv2.imshow("Detect", img)
        cv2.waitKey(1)

    def pos_chage_judgement(self, pos0, pos_det, self_conf_change, self_conf_keep, self_decision_change):
        conf_change = self_conf_change
        conf_keep = self_conf_keep
        decision_change = self_decision_change
        if ((abs(pos_det[0]) - abs(pos0[0])) ** 2 + (abs(pos_det[1]) - abs(pos0[1])) ** 2) > (4 ** 2) * 2:
            conf_change = conf_change + 1
            decision_change = False
        else:
            conf_keep = conf_keep + 1
        if conf_change >= 3:
    ...
    def pos_decision_while_enemy_is_blue(self, flags):
        if flags == 0:
            # 英雄
            self.pos_data[0] = 1600
            self.pos_data[1] = 100
            # 工程
            self.pos_data[2] = 2000
            self.pos_data[3] = 450
            # 平步
            self.pos_data[4] = 1500
            self.pos_data[5] = 100
            # 四号步兵
            self.pos_chage_judgement(self.pos_data_cache[6:8], self.pos_data[6:8],
                                     self_conf_change=self.conf_change_4,
                                     self_conf_keep=self.conf_keep_4,
                                     self_decision_change=self.decision_change_4)
            # 五号步兵
            self.pos_chage_judgement(self.pos_data_cache[8:10], self.pos_data[8:10],
                                     self_conf_change=self.conf_change_5,
                                     self_conf_keep=self.conf_keep_5,
                                     self_decision_change=self.decision_change_5)
            # 哨兵
            if not self.guess_7_decision:
                if self.choice1_7:
                    self.pos_data[10] = 2250
            ...
            # 1号英雄
            self.pos_chage_judgement(self.pos_data_cache[0:2], self.pos_data[0:2],
                                     self_conf_change=self.conf_change_1,
                                     self_conf_keep=self.conf_keep_1,
                                     self_decision_change=self.decision_change_1)
            # 工程
            self.pos_data[2] = 1500
            self.pos_data[3] = 850
            # 3号平步
            self.pos_chage_judgement(self.pos_data_cache[4:6], self.pos_data[4:6],
                                     self_conf_change=self.conf_change_3,
                                     self_conf_keep=self.conf_keep_3,
                                     self_decision_change=self.decision_change_3)
            # 四号步兵
            self.pos_chage_judgement(self.pos_data_cache[6:8], self.pos_data[6:8],
                                     self_conf_change=self.conf_change_4,
                                     self_conf_keep=self.conf_keep_4,
            ...
            self.pos_chage_judgement(self.pos_data_cache[0:2], self.pos_data[0:2],
                                     ...
                                     self_decision_change=self.decision_change_1)
            # 二号工程
            self.pos_chage_judgement(self.pos_data_cache[2:4], self.pos_data[2:4],
                                     self_conf_change=self.conf_change_2,
                                     self_conf_keep=self.conf_keep_2,
                                     self_decision_change=self.decision_change_2)
            # 三号平步
            self.pos_chage_judgement(self.pos_data_cache[4:6], self.pos_data[4:6],
                                     self_conf_change=self.conf_change_3,
                                     self_conf_keep=self.conf_keep_3,
    ...
    def pos_decision_while_enemy_is_red(self, flags):
        if flags == 0:
            # 英雄
            self.pos_data[0] = 1200
            self.pos_data[1] = 1400
            # 工程
            self.pos_data[2] = 800
            self.pos_data[3] = 1050
            # 平步
            self.pos_data[4] = 1300
            self.pos_data[5] = 1400
            # 四号步兵
            self.pos_chage_judgement(self.pos_data_cache[6:8], self.pos_data[6:8],
                                     self_conf_change=self.conf_change_4,
                                     self_conf_keep=self.conf_keep_4,
    ...
    def timer_callback(self,event):
        if self.enemy_is_blue:
            # [1600, 100, 2000, 450, 1500, 100, 1950, 1350, 1950, 1350, 2250, 750]
            # 金矿：(1500, 850) 银矿:(2000, 450)
            t1 = time.time()
            if (t1 - t0) <= 150:
                flag = 0
            elif 150 <= (t1 - t0) <= 300:
                ...
                flag = 2
            self.pos_decision_while_enemy_is_blue(flags=flag)
            self.pos_data_cache = self.pos_data
            send_interact_blue = com_class.lmap_interaction_send(self.pos_data, 0)
            com.send_data(send_interact_blue)
            t2 = time.time()
            t_wasted = t2 - t1
            print(f"浪费时间为：{t_wasted}")
            time.sleep(self.delaytime - t_wasted)
        else:
            # [1200, 1400, 800, 1050, 1300, 1400, 850, 150, 850, 150, 570, 750]
            # 金矿：(1300, 650) 银矿:(800, 1050)
            t1 = time.time()
            if (t1 - t0) <= 150:
                flag = 0
            elif 150 <= (t1 - t0) <= 300:
                ...
                flag = 2
            self.pos_decision_while_enemy_is_red(flags=flag)
            self.pos_data_cache = self.pos_data
            ...
            print(f"浪费时间为：{t_wasted}")
            time.sleep(self.delaytime - t_wasted)
    def timer_double_callback(self, event):
        if self.enemy_is_blue:
            com.send_data(com_class.double1_R_datapack)
            time.sleep(0.5)
            com.send_data(com_class.double2_R_datapack)
            time.sleep(0.5)
        else:
            com.send_data(com_class.double1_B_datapack)
            time.sleep(0.5)
            com.send_data(com_class.double2_B_datapack)
            time.sleep(0.5)
...
"""
此版本通信有问题，还在修改
问题在于对于开小陀螺或移动较快的车，误识别导致检测框移动幅度大
我们定位的方式是基于检测框定位
会导致误识别的兵种飘来飘去
"""